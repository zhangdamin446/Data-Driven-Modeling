...
!-----------------------------------------------------------------------
  imsg = 7
  idia = & ! 0 (in case stdo is preferred)         
    0
  istv = & ! reserved for state variable output file                
    425
  stress_ns(0, :) = stress(:)
...
!     restore stran_el, stran_pl, and yldp
  Call restore_statev(statev, nstatv, eeq_ns(0), stran_el_ns(0,:), stran_pl_ns(0,:), ntens, yldp_ns(0,:), nyldp, 0, .False., idia, .False., kinc, noel, npt, time(1), stress)
...
!          elastic regime
  deq_pr = & !assuming no plastic increment          
    0.
  hrdp(1) = eeq_ns(0) + deq_pr
  Call uhard(ihrd_law, hrdp, nhrdp, hrdc, nhrdc, flow_stress, dflow_stress, empa)
...
!-----------------------------------------------------------------------
  If (phi_ns(0)<flow_stress) & ! elastic                      
    Then
    Call update_elastic(idia, idiaw, iyld_law, ntens, nyldp, nstatv, ddsdde, cel, stran, stran_el_ns, stran_pl_ns, dstran, stress, eeq_ns, deq, yldp_ns, statev, kinc)
    dstran_pl(:) = 0D0
    dstran_el(:) = dstran(:)
...
!! plastic                                                   
    Call return_mapping(cel, stress_ns(1,:), phi_ns(0), eeq_ns(0), dphi_n, dstran, stran_el_ns(0,:), stran_pl_ns(0,:), ntens, idiaw, idia, hrdp, nhrdp, hrdc, nhrdc, ihrd_law, iyld_law, yldc, nyldc, yldp_ns, nyldp, stress, deeq, dstran_pl, dstran_el, statev, nstatv, ddsdde, failnr, kinc, noel, npt, time)
